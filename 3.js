...
let facePhoto = null; // 存储表情照片
let scenePhoto = null; // 存储景点照片
...
async function startCamera() {
    try {
        ...
        captureSceneBtn.disabled = false;
...
function captureFace() {
    ...
    photoCount++;
    updateCount();
    updateStatus('已拍摄表情照片');

    // 如果两张照片都已拍摄，启用分析按钮
    if (facePhoto && scenePhoto) {
...
function captureScene() {
    scenePhoto = capturePhoto();
    photoCount++;
    updateCount();
    updateStatus('已拍摄景点照片');
...
async function analyzeAndGenerate() {
    ...
    try {
        ...
        // 调用智谱AI API
        const guideContent = await callZhipuAPI(facePhoto, scenePhoto);
...
captureSceneBtn.addEventListener('click', captureScene);
